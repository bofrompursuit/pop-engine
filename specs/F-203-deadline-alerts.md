...
1. Materializing a checklist schedules the correct alert set from the plan's typed deadlines; items with `unverified` deadlines schedule nothing (no invented dates) and are listed in the checklist as "confirm lead time with agency."
2. The poller sends due alerts within 2 minutes of `send_at`, marks sent/failed, and retries failures on later ticks.
3. Hard floors are never softened: reminder copy for R2-derived deadlines states "applications within 21 days of the event are not accepted."
