...
9. **Moved computed deadline.** This criterion applies to a row the latest plan still raises, and only to one. A **dropped** row — kept and struck through by Acceptance Criterion 6 because the latest plan no longer raises the requirement — has no item in the latest plan, so there is no current date and no current deadline state for it to be compared against: the comparison below has one operand, not two. Such a row never carries a moved-deadline notice. It is not silent about anything as a result: the strike-through already says the requirement is no longer raised, and its dates remain the last plan's that did raise it, attributed to that plan, which is F-206 Acceptance Criterion 4's rule and not this one's to restate. Stated here because an implementation reading only the comparison would find no matching item and have to invent an answer.
   ...
   - **The deadline state** — what the row says about the deadline other than the date: the published deadline object **in full**, whether the finding is dated at all or is `not_calculable` or `not_applicable`, and `timeline_unresolved_reason`, `deadline_unknown_fields` and `deadline_display`. It moved when any of those differs.
     ...
     The deadline object is compared field by field rather than by `type` alone, because two plans can agree on `type` and still publish different deadlines: a lead-time or boundary correction changes `business_days`, `calendar_days` or `boundary` underneath an unchanged `type`. That gap is widest exactly where it is least visible. Where neither plan can date the finding, both `latest_apply_date`s are null and `deadline_status` is `not_calculable` on both sides, so a same-type rule edit moves nothing else this criterion looks at and the row reads as untouched. Every `business_days_minimum` finding is in that state for as long as no holiday list is published (SPEC-CONFLICT #130), so this is the present condition of several rows, not a hypothetical. The fields the published ruleset uses today are `type`, `calendar_days`, `business_days`, `boundary`, `qualification`, `display`, `level_field`, `levels`, `unknown_level_behavior`, `multi_block_field`, `hard_floor_days` and `processing_range_days`. They are named so an implementation compares a known set instead of deciding for itself which ones count; a ruleset that adds a field to a deadline adds it to this comparison.
