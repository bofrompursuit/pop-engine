...
**Phase:** 1 (core, week 1) · **Lane:** Dev 2 · **Depends on:** events schema (Phase 0), ruleset nyc.v2.6 ratified (BASELINE.md) · **Feeds:** everything (single source of truth)
**Updated:** 2026-07-22 against nyc.v2.1; retargeted to nyc.v2.5 on 2026-07-25, and to nyc.v2.6 on 2026-07-25 (dedupe-key correction only; no intake field, enum or asked-when condition moves).
...
The field list, enums, and asked-when conditions come from the ruleset's `intake_fields` registry (`rules/nyc-rules.v2.6.json`) — **the registry is authoritative; do not duplicate or drift from it.** Field groups (mirrored by the `events` table in ARCHITECTURE.md):
