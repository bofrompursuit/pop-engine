# PopEngine — Scenario Fixtures v3 (derived from ruleset nyc.v2.3)

**Status:** APPROVED (2026-07-22, ratified with `rules/nyc-rules.v2.3.json`; see `docs/BASELINE.md`). Now the green-gate acceptance suite. Individual regulatory facts still promote SOURCE_CONFIRMED → VERIFIED during the build via the ruleset's `verification` blocks (OPEN-QUESTIONS §2); that promotion is the verification owner's, per CONTRIBUTING Golden Rule 2.
