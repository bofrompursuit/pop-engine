...
**Status:** APPROVED (2026-07-22; see `docs/BASELINE.md`). Technical design for Phase 0–1.5. Companion to `PRD.md` (requirements), `ROADMAP.md` (phasing), `DESIGN.md` (lanes, gates, demo), `ARCHITECTURE-FUTURE.md` (Phase 2+ target). Permit facts referenced here trace to `rules/nyc-rules.v2.2.json`; none are asserted beyond it.
...
| AD-2 | The published ruleset file in git (`rules/nyc-rules.v2.2.json`) is authoritative; loaded in-memory at boot; `permit_rules` table is a seeded read model | Engine tests run without a database; rule changes are PRs (matches the manual rules-admin workflow in `DESIGN.md`); "crown jewel is a versioned file" stays literally true. |
...
| AD-9 | Ruleset baseline: nyc.v2.2 (corrected subset), schema popengine-rules/v2 | Adopted 2026-07-22 after two fetch-confirmed verification passes; the 59-rule draft in `rules/proposals/` is the post-capstone target. |
...
│   plan render,   │                    │  └─ packages/engine ◀── rules/nyc-rules.v2.2.json
│   stretch pages) │                    └────────┬─────────┬───────┘
└─────────────────┘                              │         │
...
/rules             nyc-rules.v2.2.json (published ruleset) + proposals/ (drafts; never build from these)
...
Intake columns mirror the ruleset's `intake_fields` registry (`rules/nyc-rules.v2.2.json` is authoritative for names, enums, and asked-when conditions; the table below groups them):
...
### permit_rules *(read model, seeded from `rules/nyc-rules.v2.2.json` at migration/boot; never hand-edited)*
...
- Environment variables per service: `DATABASE_URL`, `S3_*`, `TWILIO_*`, `SMTP_*`, `RULES_FILE` (path, defaults to `rules/nyc-rules.v2.2.json`), `API_BASE_URL` / `WEB_ORIGIN` (CORS).
...
- **Rules loading:** api boots by validating `rules/nyc-rules.v2.2.json` (schema check; 33 rules + 4 advisories present; every trigger field declared in the file's `intake_fields` registry) and syncing `permit_rules`; a validation failure aborts boot loudly.
