...
| T-4 | Whether three scope claims are one axis measured at three points or three that merely rhyme: `VerificationStatus.COVERAGE_GAP` ("combination not modeled by this ruleset version"; per rule, shipped), `ARCHITECTURE-FUTURE.md` §7.1's `OUTSIDE_VALIDATED_COVERAGE` (per result, after evaluation), and F-109's "unsupported" (per request, before evaluation) | Product owner | **Blocks F-109 and F-601. Not the shipped engine:** `COVERAGE_GAP` works today and §7.1 is implemented nowhere, so no current work waits on this. The three names stay deliberately distinct until either F-109 or §7.1 is implemented; whichever lands first must decide whether one field with a level discriminator replaces them, and that decision closes this row. PR #136 renamed §7.1's concept to "result completeness" and F-109's to "scope support states", so the names no longer collide — what is left is the substantive question, not the collision. Its earlier framing understated the overlap, because it rested on reading `COVERAGE_GAP` as a missing-source marker rather than a not-modeled one. |
